...
const Cart = () => {
  ...
  // ✅ Correctly access cart state from Redux
  const cart = useSelector((state: RootState) => state.cart);

  // ✅ Prepare product list with default image fallback
  const products: Product[] = useMemo(
    () =>
      cart.map(product => ({
        ...product,
        src: (product as Product).src || '/default-image.jpg',
      })),
    [cart]
  );
    ...
    <div className="w-screen overflow-scroll md:w-[45vw] h-screen p-5 top-11 right-0 absolute z-10 bg-stone-100 text-black shadow-2xl shadow-stone-950 transition-opacity opacity-0 animate-fade-in space-y-2 delay-100">
      {products.length > 0 ? (
        ...
        <div className="text-center space-y-9">
          ...
          <Link to="/products">
            <button
              className="border-2 w-[80%] mx-[10%] mt-9 py-3 uppercase bg-[#ffbf00] hover:bg-[#ffa500] text-stone-900 rounded-md text-sm font-semibold"
      ...
      {products.map(product => (
        <div
          key={product.key}
          className="h-[120px] shadow p-2 text-stone-800 bg-white border-gray-300 w-[97%] justify-around mx-auto text-center md:text-left flex items-center"
        >
          ...
          <div className="flex items-center space-x-2 w-[15%]">
            <IoMdRemove
              ...
              onClick={() => dispatch(decreaseQuantity(product.key))}
            />
            <span>{product.quantity}</span>
            <IoMdAdd
              className="text-[#ffa500] cursor-pointer"
              onClick={() => dispatch(increaseQuantity(product.key))}
            />
          </div>
      ...
      {products.length > 0 && (
        <div className="font-semibold text-xl pt-2 border-t-4 border-stone-500">
          ...
          <Link to="/checkout">
            <button
              onClick={() => dispatch(toggleCart())}
              className="border-2 w-[90%] mx-[5%] mt-9 py-3 uppercase bg-[#ffbf00] hover:bg-[#ffa500] text-stone-900 rounded-md text-sm font-semibold"
            >
              Proceed to Checkout
            </button>
          </Link>
